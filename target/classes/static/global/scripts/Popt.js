...
var Iput = {
    ...
    show: function () {
        if(Iput.isTimer.istimer != 0){
            clearInterval(Iput.isTimer.istimer);
            $("#machineTypeSet").val("");
        }
    ...
    colse: function () {
        if(Iput.isMap.ismap == 'yes'){
             var locate = $('.map-search-locate').val();
            $('.map-search-locate').attr('title',locate)
            // var end=locate.lastIndexOf("-", locate.lastIndexOf("-")+1)+1;
            // var key=locate.slice(end);
            // $('.map-search-locate').val(key);
        ...
        if(Iput.isAgri.isargi == 'group'){
            $('#org-group').val($('#orgId').val());
        }else if(Iput.isAgri.isargi == 'move'){
            $('#org-move').val($('#orgId').val());
        }else if(Iput.isAgri.isargi == 'notprovince'){
	        if($('#orgId').val() != undefined && $('#orgId').val() != ""){
	        	$.ajax({
	        		url:'manage/cooperative/verifyOrg',
	                type:'POST',
	                data:{
	                    orgId: $("#cooperationForm").find("#orgId").val()
	                },
	                dataType:'json',
	                success:function (data) {
	                    if(!data){
	                        $('.coop-validate-container').addClass('has-error');
	                        $('.coop-vaidate').text('不能在省级分组及合作社下添加该信息，请重新选择！').addClass('help-block-error')
	                    }else{
	                        $('.coop-validate-container').removeClass('has-error');
	                        $('.coop-vaidate').text('').removeClass('help-block-error')
	                    }
                    }
                })
            }
	        Iput.isAgri.isargi = '';
        }else if(Iput.isAgri.isargi == 'organization'){
	    	...
	    	/*if($('#orgId').val() != undefined && $('#orgId').val() != ""){
	    		$.ajax({
	        		url:'manage/org/verifyParentOrg',
	                type:'POST',
	                data:{
	                    orgId: $("#orgId").val()
	                },
	                ...
	                success:function (data) {
	                    if(!data){
	                        $('.org-validate-container').addClass('has-error');
	                        $('.org-vaidate').text('不能在合作社下添加该信息，请重新选择！').addClass('help-block-error')
	                    }else{
	                        $('.org-validate-container').removeClass('has-error');
	                        $('.org-vaidate').text('').removeClass('help-block-error')
	                    }
	                }
	            })
	        }
 	        Iput.isAgri.isargi = '';*/
        }else if(Iput.isAgri.isargi == 'verifyOrgAndGetAddr'){
            if($('#orgId').val() != ""){
                $.ajax({
                    url:'manage/agriMachInfo/verifyOrg',
                    type:'POST',
                    data:{
                        orgId: $('#orgId').val()
                    },
                    ...
                    success:function (data) {
                        if(!data.verify){
                            $(".agri-validate-container").find('.help-block').text('').removeClass('help-block-error');
                             $('.agri-validate-container').addClass('has-error');
                             $('.agri-vaidate').text('不能在省级分组下添加该信息，请重新选择！').addClass('help-block-error')
                             }else{
                             $('.agri-validate-container').removeClass('has-error');
                             $('.agri-vaidate').text('').removeClass('help-block-error');
                             $(".addr").val(data.addr);
                             $(".carsAdds").val(data.addr);
                         }
                    }
                })
            }
            Iput.isAgri.isargi = '';
        }else if(Iput.isAgri.isargi == 'account'){
            $('#roleIds').hide();
            if($('#orgId').val() != "" ){
                    $.ajax({
                        url: "manage/account/getRoleName",
                        type: 'POST',
                        data:{
                            orgId:$("#accountFrom").find('#orgId').val(),
                            roleId:$("#accountFrom").find("#userId").val()
                        },
                        success: function (response) {
                            $("#roleSIds").empty();
                            if( response.length > 0) {
                                $.each(response, function (index,item) {
                                    if('admin' != item.roleSign){
                                        $('.agri-validate-role').removeClass('has-error');
                                        $('.agri-vaidate').text('').removeClass('help-block-error');
                                        $("#roleSIds").append('<option value="'+item.roleId+'">'+item.roleName+'</option>');

                                    }else{
                                        $('.agri-validate-role').addClass('has-error');
                                        $('.agri-vaidate').text('当前分组不允许创建用户，请重新选择分组！').addClass('help-block-error')
                                    }
                                });
                            }else{
                                $('.agri-validate-role').addClass('has-error');
                                $('.agri-vaidate').text('当前组织无角色,请先创建角色！').addClass('help-block-error')
                            }
                        }
                    });
            }
            Iput.isAgri.isargi = '';
        }else if(Iput.isAgri.isargi == 'machineTypeVerify'){
            if(Iput.typeId.typeid != 0){
               $("#machineType").val(Iput.typeId.typeid);
            }else{
                $("#machineType").val("");
            }
            //clearInterval(Iput.isTimer.istimer);
            var machineTypeId = $("#machineType").val();
            if(machineTypeId == "" || machineTypeId == null){
                $("#machineTypeSet").val("");

            }else {
                $("#machineTypeSet").val(Iput.typeVlue.typevlue);
                $("#machineTypeSet").removeAttr("style");
            }
        }
        Iput.isAgri.isargi = '';
        Iput.isTimer.istimer = 0;
        /*Iput.typeId.typeid = 0;
        Iput.typeVlue.typevlue = '';*/
    },
